...
export interface CommunityPostCursorPayload {
  v: 1;
  createdAt: string;
  id: string;
}
...
/**
 * Encodes a versioned, opaque cursor from a post's createdAt timestamp and UUID.
 */
export function encodeCursor(post: { createdAt: Date | string; id: string }): string {
  const createdAtStr =
 ...
 */
export function decodeCursor(cursorStr: string): { createdAt: Date; id: string } {
