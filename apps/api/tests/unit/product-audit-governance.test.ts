...
describe("FEAT-016 Product Audit Governance & Abstraction Unit Tests", () => {
  describe("Product Audit Governance Scope Guard (DEF-003)", () => {
    ...
    it("proves guard catches injected false-negative Prisma models (model Ai, model AI, renamed/mapped/domain models)", () => {
      ...
      // 5. Probed unapproved future domain model Community
      const probeCommunity = evaluateProductAuditGovernance({
        schemaContent: `
          model Community {
            id String @id
          }
      ...
      expect(probeCommunity.passed).toBe(false);
      expect(probeCommunity.violations.some((v) => v.includes("Community"))).toBe(true);
    });
    ...
    it("proves guard catches injected false-negative SQL migration statements (quoted/camelCase/snake_case tables)", () => {
      ...
      // 3. Unapproved domain table "community_posts"
      const probe3 = evaluateProductAuditGovernance({
        migrationSqls: [{ name: "20260901_test", sql: 'CREATE TABLE "community_posts" (id UUID PRIMARY KEY);' }],
      });
      expect(probe3.passed).toBe(false);
      expect(probe3.violations.some((v) => v.includes("community_posts"))).toBe(true);
    });
